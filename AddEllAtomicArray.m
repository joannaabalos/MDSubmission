function AddEllAtomicArray(rad, X0, Y0, VX0, VY0, InitDist, Temp, Type)
global C
...
W = (6*rad - 1) * AtomSpacing;

xp(1, :) = linspace(-L/2, L/2, 2*rad);
yp(1, :) = linspace(-W/2, W/2, 6*rad);

numAtoms = 0;
for i = 1:2*rad
    for j = 1:6*rad
        if (xp(i)^2 + yp(j)^2 <= (rad*AtomSpacing)^2) && (xp(i)^2 + yp(j)^2 >= (rad*5/6*AtomSpacing)^2) 
            numAtoms = numAtoms+1;
...
for i = 1:2*rad
    for j = 1:6*rad
        if (xp(i)^2 + yp(j)^2 <= (rad*3.7/6*AtomSpacing)^2) && (xp(i)^2 + yp(j)^2 >= (rad*3.5/6*AtomSpacing)^2) 
            numAtoms = numAtoms+1;
            x(nAtoms + numAtoms) = xp(i);
            y(nAtoms  + numAtoms) = yp(j);
        else
...
for i = 1:2*rad
    for j = 1:6*rad
        if (xp(i)^2 + yp(j)^2 <= (rad/2*AtomSpacing)^2) && (xp(i)^2 + yp(j)^2 >= (rad*2/6*AtomSpacing)^2) 
            numAtoms = numAtoms+1;
            x(nAtoms + numAtoms) = xp(i);
            y(nAtoms  + numAtoms) = yp(j);
        else
...
for i = 1:2*rad
    for j = 1:6*rad
        if (xp(i)^2 + yp(j)^2 <= (rad/6*AtomSpacing)^2) 
            numAtoms = numAtoms+1;
            x(nAtoms + numAtoms) = xp(i);
            y(nAtoms  + numAtoms) = yp(j);
        else
